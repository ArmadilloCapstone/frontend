...
function BbsUpdate() {
	...
	useEffect(() => {
		...
		const getBoard = async () => {
		  const {data} = await axios.post(`http://localhost:80/news/${param.bbs_id}`);
		  return data;
		}
		getBoard().then((result) => {
		  ...
		  setImageUrl({...imageUrl, preview_URL: `http://localhost:80/news/image/${param.bbs_id}`})
		});
		
        axios.post(`http://localhost:80/news/files/${param.bbs_id}`)
            .then((res) => {
	...
	const updateBbs = async (e) => {
		...
		if(fileChange){
			...
			axios.post("http://localhost:80/BbsList/update/file", formData, {
			  headers: {
			...
			axios.post("http://localhost:80/BbsList/update/nofile", formData, {
			  headers: {
				"Content-Type": "multipart/form-data",
			  }
			})
