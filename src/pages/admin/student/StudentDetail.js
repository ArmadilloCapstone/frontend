...
function StudentDetail() {
  ...
  const loadStudentDetail = async () => {
    await axios.post('http://localhost/student') // student와 parent를 연결해서 어떻게 데이터를 가져오는 거지? db 쿼리로 처리하는 건가?
  ...
  const submitStudentRecord = async (e) => {
    e.preventDefault();
    e.target.reset();
    await axios.post("http://localhost/student_submit", user);
    alert('추가되었습니다!');
  ...
  const deleteRecord = (productId) => {
    axios.delete(`http://localhost/student/${productId}`)
      .then((result) => {
        loadStudentDetail();
      })
      .catch(() => {
